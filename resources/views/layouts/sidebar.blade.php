...
<body>
    <div class="offcanvas offcanvas-start w-25" tabindex="-1" id="offcanvas" data-bs-keyboard="false" style="background-color: #003459; color:white;">
    ...
    <div class="offcanvas-body px-0 py-0"  style="position: relative;">
        <div  style="max-height: 70vh; overflow-x: hidden; overflow-y: auto;">
            ...
            <li class="nav-item-dropdown w-100 " >
                <a href="#usermenu" data-bs-toggle="collapse" class="nav-link text-truncate " style="font-size: 18px;">
                ...
                <ul class="collapse nav flex-column ms-3 mt-1" id="usermenu" data-bs-parent="#menu">
                    ...
                    <li class="nav-item w-100 " >
                        <a href="{{ url('pemain', []) }}" class="nav-link text-truncate " style="font-size: 18px;">
                            <i class="fs-5 bi bi-person-fill"></i>
            ...
            <div class="dropdown dropup " >
                <a href="#" class="drop-profil clicked d-flex align-items-center text-decoration-none dropdown-toggle ps-2 pe-2 pt-2 pb-2" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
                    @if (Auth::check())
                        ...
                        @else
                            <img src="{{ asset('storage/' . Auth::user()->foto_profil) }}" class="rounded-circle" style="width: 40px;" />
                        @endif
                    @endif
                    <span class="d-none d-sm-inline ms-3 text-capitalize overflow-hidden w-75 " style="font-size: 18px; white-space: nowrap; text-overflow: ellipsis">
                        @if (Auth::check())
                            {{Auth::user()->username}}
                        @endif
                    </span>
                        ...
                        <div class="col">
                            <div class="d-flex align-items-center">
                                @if (Auth::check())
                                    ...
                                    @else
                                        <img src="{{ asset('storage/' . Auth::user()->foto_profil) }}" class="rounded-circle ms-3" style="width: 60px;" />
                                    @endif
                                    <div class="ms-0 row">
    ...
    <div class="shadow-sm p-2 mb-5 bg-body">
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 d-none d-lg-flex d-md-flex-d-sm-flex d-xs-none justify-content-end " >
                ...
                <div class="d-flex align-items-center " style="background-color: ; font-size: 16px; ">
                    ...
                    <div class="nav-item-top">
                        <a class="nav-link mx-2 text-uppercase " aria-current="page" href="{{ url('berita', []) }}">Berita</a>
                    </div>

                    <div class="nav-item-top">
                        <a class="nav-link mx-2 text-uppercase " aria-current="page" href="{{ url('galeri', []) }}">Galeri</a>
                    </div>
                </div>
                
            @if (!Auth::check())
            <div class="d-flex align-items-center pe-0 ms-3">
                <a href="{{ url('login', []) }}" class="btn btn-primary  d-flex align-items-center mt-1 "  > 
                    Login
                </a>
            </div>
                @else
                <button class="btn py-0 mt-1 ms-3" style=" background-color:#003459; width:50px; height:50px;" data-bs-toggle="offcanvas" data-bs-target="#offcanvas" role="button">
                    {{-- <span class="text-white fs-4 " style="vertical-align: middle;" >Menu</span> --}}
                    <i class="bi bi-list fs-3" style="color:white; vertical-align: middle;"  data-bs-toggle="offcanvas" data-bs-target="#offcanvas"></i>
                </button>
            @endif
